...
public class GroupEditorScreen extends Screen {
	...
	private void renderActionTitle(GuiGraphicsExtractor g, EditorShellLayout.Rect rect) {
		...
		g.text(font, clipped, rect.x(), UiSkinRenderer.centeredTextY(font, rect.y(), rect.height()),
	...
	private void renderDisableSourceOption(GuiGraphicsExtractor g, int mouseX, int mouseY) {
		...
		g.text(font, clipped, label.x(), UiSkinRenderer.centeredTextY(font, label.y(), label.height()),
	...
	private List<GroupPreviewEntry> settingsPreviewEntries() {
		...
		for (var generic : rightPanel.groupGeneric()) {
			if (generic.presentationData() instanceof mezz.jei.api.ingredients.ITypedIngredient<?> typed) {
				entries.addAll(GroupPreviewEntry.fromTypedIngredients(List.of(typed)));
			}
		}
		return entries;
	}

	private GroupSampleRenderer.Rect sampleRect(EditorShellLayout.Rect area) {
	...
	private void renderFullHeaderPreview(GuiGraphicsExtractor g, EditorShellLayout.Rect row) {
		int x = row.x();
		int y = row.y() + Math.max(0, (row.height() - HEADER_PREVIEW_SIZE) / 2);
		renderHeaderPreviewBox(g, x, y);
		...
		g.text(font, clipped, textX, textY, UiPalette.TEXT_PRIMARY, false);
		String summary = font.plainSubstrByWidth(fullPreviewSummary(), maxWidth);
		g.text(font, summary, textX, textY + font.lineHeight + 2, UiPalette.TEXT_MUTED, false);
	}
	...
	private void renderHeaderButton(GuiGraphicsExtractor g, EditorShellLayout.Rect rect, String label, boolean active,
	...
	private void renderFieldChrome(GuiGraphicsExtractor g, EditorShellLayout.Rect rect, boolean focused, boolean hovered) {
