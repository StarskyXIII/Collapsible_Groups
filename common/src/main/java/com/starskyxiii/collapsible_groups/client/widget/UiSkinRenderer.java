...
public final class UiSkinRenderer {
    ...
    public static void drawToolbarChevronButton(GuiGraphics g, int x, int y, boolean right, ButtonState state) {
        ...
        g.blitSprite(toolbarButtonSprite(state), x, y + offset, TOOLBAR_BUTTON_WIDTH, TOOLBAR_BUTTON_HEIGHT);
	...
	private static void drawCheckboxMark(GuiGraphics g, int x, int y, int color) {
	...
	public static void drawScreenBars(GuiGraphics g, int width, int height, int headerHeight, int footerHeight) {
		...
		g.fill(0, headerHeight, width, headerHeight + 1, UiPalette.DIVIDER);
		g.fill(0, footerY, width, height, UiPalette.SCREEN_BAR_SHADOW);
		g.fill(0, footerY, width, footerY + 1, UiPalette.DIVIDER);
	}
	...
	public static void drawButton(GuiGraphics g, Font font, int x, int y, int width, int height,
		...
		g.blitSprite(buttonSprite(state), x, y, width, height);
	...
	private static int buttonTextColor(ButtonState state) {
		return switch (state) {
			...
			case NORMAL -> UiPalette.TEXT_DARK;
		};
	}
	...
	public static void drawScrollbarPixels(GuiGraphics g, int x, int y, int height,
		...
		g.fill(x + 2, y, x + 4, y + height, UiPalette.SCROLLBAR_TRACK_LINE);
	...
	public static void drawMiniScrollbar(GuiGraphics g, int x, int y, int height,
		...
		g.fill(x + 2, y, x + 3, y + height, UiPalette.SCROLLBAR_TRACK_LINE);
	...
	public static void drawSlot(GuiGraphics g, int x, int y, int size) {
		g.fill(x, y, x + size, y + size, UiPalette.SURFACE_DARK);
		drawOutline(g, x, y, size, size, UiPalette.OUTLINE_DARK);
	}
	...
	public static EditorChrome.Rect drawSlider(GuiGraphics g, int x, int y, int width, int value, int max,
		...
		g.fill(x, trackY, x + width, trackY + SLIDER_TRACK_HEIGHT,
			active ? SLIDER_TRACK_UNFILLED : UiPalette.SURFACE_DISABLED);
		g.fill(x, trackY, x + Math.max(1, (int) ((long) width * clamped / Math.max(1, max))),
			trackY + SLIDER_TRACK_HEIGHT,
			active ? UiPalette.BUTTON_PRIMARY : UiPalette.OUTLINE_DISABLED);
		drawOutline(g, x, trackY, width, SLIDER_TRACK_HEIGHT,
			hot ? UiPalette.OUTLINE_HOVER : UiPalette.OUTLINE_DARK);
		...
		if (active) {
			...
			g.fill(kx + 1, ky + 1, kRight - 1, kBottom - 3,
				hot ? UiPalette.BUTTON_LIGHT_HOVER : UiPalette.BUTTON_LIGHT);
			// … with its light inner ring on all four sides …
			drawOutline(g, kx + 1, ky + 1, knob.width() - 2, knob.height() - 4, UiPalette.BUTTON_LIGHT_TOP);
			// … the dark bottom bevel …
			g.fill(kx + 1, kBottom - 3, kRight - 1, kBottom - 1, UiPalette.BUTTON_LIGHT_BOTTOM);
			// … and the dark frame (disjoint from the inset regions above).
			drawOutline(g, kx, ky, knob.width(), knob.height(), UiPalette.OUTLINE_DARK);
			if (hot) {
				// Bedrock-style focus ring just outside the frame.
				drawOutline(g, kx - 1, ky - 1, knob.width() + 2, knob.height() + 2, UiPalette.OUTLINE_HOVER);
			}
		}
		return knob;
	}
	...
	public static void drawSlotGrid(GuiGraphics g, int x, int y, int cols, int rows, int cellPitch) {
		...
		for (int col = 0; col <= cols; col++) {
			...
			g.fill(lineX, y, lineX + 1, y + height, UiPalette.OUTLINE_DARK);
		}
		for (int row = 0; row <= rows; row++) {
			int lineY = y + row * cellPitch;
			g.fill(x, lineY, x + width, lineY + 1, UiPalette.OUTLINE_DARK);
		}
	}
	...
	public static void drawRemoveBadge(GuiGraphics g, int iconX, int iconY, boolean hovered) {
		...
		if (hovered) {
			drawOutline(g, bx, by, r.width(), r.height(), UiPalette.OUTLINE_HOVER);
		}
