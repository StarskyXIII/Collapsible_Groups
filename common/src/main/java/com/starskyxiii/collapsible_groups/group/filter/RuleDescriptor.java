...
public record RuleDescriptor(
	...
	private static Map<FilterNodeKind, RuleDescriptor> createTable() {
		...
		table.put(FilterNodeKind.BLOCK_TAG, oneValue(ReferencePickerSource.BLOCK_TAGS));
		table.put(FilterNodeKind.ITEM_PATH_STARTS_WITH, oneValue(ReferencePickerSource.NONE));
		table.put(FilterNodeKind.ITEM_PATH_CONTAINS, oneValue(ReferencePickerSource.NONE));
		table.put(FilterNodeKind.ITEM_PATH_ENDS_WITH, oneValue(ReferencePickerSource.NONE));
		table.put(FilterNodeKind.NAMESPACE, atomic(true, TypePolicy.INGREDIENT_TYPED,
			List.of(FieldRole.INGREDIENT_TYPE, FieldRole.PRIMARY_VALUE), Set.of(FieldRole.PRIMARY_VALUE),
			ReferencePickerSource.INGREDIENT_NAMESPACES));
		table.put(FilterNodeKind.EXACT_STACK, oneValue(ReferencePickerSource.ITEM_STACKS));
		table.put(FilterNodeKind.HAS_COMPONENT, atomic(true, TypePolicy.ITEM_ONLY,
		...
		table.put(FilterNodeKind.COMPONENT_PATH, atomic(true, TypePolicy.ITEM_ONLY,
	...
	private static RuleDescriptor oneValue(ReferencePickerSource pickerSource) {
		return atomic(true, TypePolicy.ITEM_ONLY, List.of(FieldRole.PRIMARY_VALUE),
