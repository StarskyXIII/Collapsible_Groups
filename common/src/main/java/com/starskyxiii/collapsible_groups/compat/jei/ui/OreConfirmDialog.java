...
public final class OreConfirmDialog {
	...
	public static void render(GuiGraphics g, Font font, int screenWidth, int screenHeight,
		...
		for (int i = 0; i < bodyLines.size(); i++) {
			drawCenteredLine(g, font, bounds, bodyLines.get(i).getString(), bodyY + i * 12, OreUiPalette.TEXT_MUTED);
		}

		Rect primary = primaryButton(screenWidth, screenHeight);
		Rect secondary = secondaryButton(screenWidth, screenHeight);
		OreUiRenderer.drawButton(g, font, primary.x(), primary.y(), primary.width(), primary.height(),
