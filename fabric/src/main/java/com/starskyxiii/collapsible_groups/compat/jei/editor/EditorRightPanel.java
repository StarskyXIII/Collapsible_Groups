...
final class EditorRightPanel {
	...
	void rebuild() {
		...
		groupFluids = EditorFluidIngredientHelper.buildViews(
			GroupRegistry.resolveFluids(temp), "FabricEditorRightPanel.buildFluidViews");
		groupGenericIngredients = EditorGenericIngredientHelper.buildViews(
			GroupRegistry.resolveGenericIngredients(temp), "FabricEditorRightPanel.buildGenericViews");
	}
	...
	boolean mouseClicked(double mouseX, double mouseY, int button, EditorLayout layout, List<ItemStack> allItems) {
		...
		for (int visRow = 0; visRow < layout.rightRows(); visRow++) {
			...
			if (sections.isItemRow(vRow)) {
				...
				ItemStack stack = groupItems.get(idx);
				if (com.mojang.blaze3d.platform.InputConstants.isKeyDown(net.minecraft.client.Minecraft.getInstance().getWindow(), 341)
					|| com.mojang.blaze3d.platform.InputConstants.isKeyDown(net.minecraft.client.Minecraft.getInstance().getWindow(), 345)) {
