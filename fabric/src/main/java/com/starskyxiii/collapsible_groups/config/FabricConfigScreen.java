...
public class FabricConfigScreen extends Screen {
	...
	private void addIntegerField(int cx, int contentY, String labelKey, int value, Consumer<Integer> setter) {
		...
		field.setResponder(text -> {
			String sanitized = digitsOnly(text);
			if (!sanitized.equals(text)) {
				field.setValue(sanitized);
				return;
			}
			setter.accept(readNonNegativeInt(sanitized, 0));
		});
	...
	public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float pt) {
		...
		for (int i = 0; i < integerFields.size(); i++) {
			...
			g.text(this.font, labelText, fieldLabelX, labelY, 0xFFFFFFFF, false);
			field.extractRenderState(g, mx, my, pt);
		}
