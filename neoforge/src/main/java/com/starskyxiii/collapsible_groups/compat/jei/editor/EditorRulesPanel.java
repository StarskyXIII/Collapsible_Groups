...
final class EditorRulesPanel {
    ...
    private void renderTree(GuiGraphicsExtractor g, int mouseX, int mouseY, List<RuleRow> rows) {
        ...
        try {
            for (RuleRow row : rows) {
                ...
                for (FormattedCharSequence line : row.lines()) {
                    g.text(font, line, lx, ly, row.selected() ? COL_DEFAULT : COL_TEXT, false);
                    ly += font.lineHeight;
                }
                y += rh;
            }
    ...
    private void renderScrollbar(GuiGraphicsExtractor g, List<RuleRow> rows) {
    ...
    private void renderSidebar(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        g.text(font, Component.translatable(ModTranslationKeys.EDITOR_PANEL_RULES_HEADER).getString(),
            acts.x() + PAD, acts.y() + PAD, COL_LABEL, false);
        if (btnAdd    != null) btnAdd.extractRenderState(g, mouseX, mouseY, 0);
        if (btnDelete != null) btnDelete.extractRenderState(g, mouseX, mouseY, 0);
    ...
    private void renderConfigureHeader(GuiGraphicsExtractor g, EditorChrome.Rect cfg, GroupFilterRuleDraft.Node sel) {
        int x = cfg.x() + PAD;
        int y = cfg.y() + PAD;

        for (FormattedCharSequence line : configureTitleLines(sel)) {
            g.text(font, line, x, y, COL_LABEL, false);
            y += font.lineHeight;
        }
        ...
        if (!contextLines.isEmpty()) {
            ...
            for (FormattedCharSequence line : contextLines) {
                g.text(font, line, x, y, COL_META, false);
                y += font.lineHeight;
            }
        }
    }

    private void renderField(GuiGraphicsExtractor g, int mx, int my, EditBox field) {
        if (field == null || !field.visible) return;
        field.extractRenderState(g, mx, my, 0);
    }
    ...
    private void renderStatus(GuiGraphicsExtractor g) {
        ...
        g.text(font, text, r.x() + PAD, ty, color, false);
    }

    private void renderModal(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        // Dim the body area
        ...
        g.enableScissor(content.x(), content.y(), content.right(), content.bottom());
        try {
            g.text(font, Component.translatable(ModTranslationKeys.EDITOR_RULES_COMPOUND_HEADER).getString(),
                col0, lblY, COL_LABEL, false);
            g.text(font, Component.translatable(ModTranslationKeys.EDITOR_RULES_BASIC_HEADER).getString(),
                col1, lblY, COL_LABEL, false);

            for (Button b : insertButtons) {
                if (b.visible) b.extractRenderState(g, mouseX, mouseY, 0);
            }

            boolean showWrap = state.selectedRuleNode() != null;
            if (showWrap) {
                int wrapTop = modalWrapHeaderY(m) - modalScrollOffset;
                g.text(font, Component.translatable(ModTranslationKeys.EDITOR_RULES_WRAP_HEADER).getString(),
    ...
    private void drawBorder(GuiGraphicsExtractor g, EditorChrome.Rect r, int color) {
