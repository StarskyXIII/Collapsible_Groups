...
public final class CGGroupSourceBuilder {
	...
	public CGGroupSourceBuilder add(String id, String name, GroupFilter filter) {
		...
		groups.add(new KubeJsLoweredGroup(groupId, name, KubeJsLoweringResult.exact(filter, publicationSource)));
		return this;
	}
	...
	public GroupFilter itemId(String id) {
		return Filters.itemId(requireId(id, "item").toString());
	}

	public GroupFilter itemTag(String id) {
		return Filters.itemTag(requireId(id, "item tag").toString());
	}
	...
	public GroupFilter fluidId(String id) {
		return Filters.fluidId(requireId(id, "fluid").toString());
	}

	public GroupFilter fluidTag(String id) {
		return Filters.fluidTag(requireId(id, "fluid tag").toString());
	}

	public GroupFilter any(Object filters) {
		return composition(filters, true);
	}

	public GroupFilter all(Object filters) {
		return composition(filters, false);
	}

	public GroupFilter not(GroupFilter filter) {
		if (!KubeJsFilterComposition.supportsTree(filter)) {
			throw new IllegalArgumentException("not() requires a supported CG filter");
		}
		return Filters.not(filter);
	}

	List<KubeJsLoweredGroup> groups() {
