...
A group created from the editor’s blank form uses `"schema_version": 1`. Editing an existing group keeps its source format, as do supported copy actions for built-in, resource-pack, and KubeJS groups. Unversioned groups stay unversioned; there is no automatic upgrade or upgrade tool.
